...
public class NikuniPompaProductService {

    private final NikuniPompaProductRepository productRepository;
    private final NikuniPompaProductTranslationRepository translationRepository;
    private final NikuniPompaProductFeatureRepository featureRepository;
    private final NikuniPompaProductCatalogRepository catalogRepository;
    ...
    public ProductDTO getProductById(Long id) {
        NikuniPompaProduct product = productRepository.findById(id)
    ...
    public ProductDTO getProductBySlug(String slug, String langCode) {
        ...
        NikuniPompaProductTranslation translation = translationRepository.findBySlugAndLanguage(slug, language)
    ...
    public ProductDTO saveOrUpdateFullProduct(ProductDTO dto) {
        NikuniPompaProduct product = saveOrUpdateSingleProduct(dto, null);
        return toDTO(product, 0);
    }

    // Tek bir ürün kaydet / güncelle (parentId ile birlikte)
    private NikuniPompaProduct saveOrUpdateSingleProduct(ProductDTO dto, NikuniPompaProduct parent) {
        NikuniPompaProduct product = (dto.getId() != null)
                ? productRepository.findById(dto.getId()).orElse(new NikuniPompaProduct())
                : new NikuniPompaProduct();
        ...
        if (dto.getParentId() != null) {
            NikuniPompaProduct parentProduct = productRepository.findById(dto.getParentId())
        ...
        if (dto.getTranslations() != null) {
            for (var t : dto.getTranslations()) {
                ...
                NikuniPompaProductTranslation trans = new NikuniPompaProductTranslation();
                ...
                Long excludeTranslationId = (dto.getId() != null) ? 
                    translationRepository.findByProductAndLanguage(product, lang)
                        .map(NikuniPompaProductTranslation::getId).orElse(null) : null;
                
                slug = SlugGenerator.ensureUniqueSlug(
                    slug,
                    s -> {
                        Optional<NikuniPompaProductTranslation> existingOpt = translationRepository.findBySlug(s);
                        if (existingOpt.isPresent()) {
                            NikuniPompaProductTranslation existing = existingOpt.get();
        ...
        if (dto.getFeatures() != null) {
            for (var f : dto.getFeatures()) {
                ...
                NikuniPompaProductFeature feature = new NikuniPompaProductFeature();
        ...
        if (dto.getCatalogs() != null) {
            for (var c : dto.getCatalogs()) {
                NikuniPompaProductCatalog catalog = new NikuniPompaProductCatalog();
    ...
    public void deleteProduct(Long id) {
        ...
        // Alt ürünleri de sil
        for (NikuniPompaProduct child : product.getChildren()) {
    ...
    public String uploadFile(MultipartFile file) {
        try {
            ...
            if (isPdf) {
                ...
                return "/api/nikuni-pompa/products/files/" + urlSafePublicId;
            }
    ...
    public byte[] getPdfFile(String urlSafePublicId) throws IOException {
        try {
            ...
            // Örnek: nikunipompa_products_ciyltgjke8yhpflxguay -> nikunipompa/products/ciyltgjke8yhpflxguay
    ...
    public void reorderProducts(List<ReorderItem> items) {
        for (ReorderItem item : items) {
            NikuniPompaProduct product = productRepository.findById(item.getId())
    ...
    private boolean isDescendantOf(Long ancestorId, Long descendantId) {
        ...
        NikuniPompaProduct current = productRepository.findById(descendantId).orElse(null);
        int depth = 0;
    ...
    // 🔸 DTO dönüşümü
    private ProductDTO toDTO(NikuniPompaProduct product, int level) {
