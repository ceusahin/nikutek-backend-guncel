...
import com.example.nikutek.repository.NikuniPompaMainHeroRepository;
...
public class NikuniPompaMainHeroService {

    private final NikuniPompaMainHeroRepository mainHeroRepository;
    ...
    public List<MainHeroDTO> getAllByLanguageCode(String code) {
        ...
        List<NikuniPompaMainHero> heroes = mainHeroRepository.findByLanguageId(language.getId());
        return heroes.stream().map(this::toDto).collect(Collectors.toList());
    }

    public MainHeroDTO toDto(NikuniPompaMainHero hero) {
    ...
    public NikuniPompaMainHero saveOrUpdate(Long id,
                                 String header,
                                 String paragraph,
                                 String button1Text,
                                 String button1Url,
        ...
        NikuniPompaMainHero hero = id != null ?
                mainHeroRepository.findById(id).orElse(new NikuniPompaMainHero()) :
                new NikuniPompaMainHero();
        ...
        if (image != null && !image.isEmpty()) {
            try {
                Map uploadResult = cloudinary.uploader().upload(image.getBytes(),
                        ObjectUtils.asMap("folder", "nikunipompa/main_hero"));
    ...
    public void deleteImage(Long id) {
        NikuniPompaMainHero hero = mainHeroRepository.findById(id)
